...
def parse_args():
    ...
    parser.add_argument('--positive_video_path', type=str)
    parser.add_argument('--negative_video_path', type=str)
    ...
    with tqdm(total=len(prompts['goal'])+len(prompts['belief'])+len(prompts['actions'])) as pbar:
        for goal in ['goal', 'belief', 'actions']:
            for cuid in prompts[goal]:
                ...
                if goal == 'goal':
                    user_text = "What is C's future goal?"
                elif goal == 'belief':
                    user_text = "At the end of these actions, what does C most likely believe?"
                elif goal == 'actions':
                    user_text = "What will C most likely do next?"


                frames_dir = os.path.join(args.positive_video_path, f"{cuid}_context")
                ...
                np.save(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal, 'correct', f'{cuid}_attn.npy'), attn_list)


                frames_dir = os.path.join(args.negative_video_path, f"{cuid}_context")
                frame_paths = sorted(glob.glob(os.path.join(frames_dir, "*.png")))
                total_frames = len(frame_paths)
                ...
                vlm.create_conversation("", user_text, clip)
                attn_list = vlm.forward()
                np.save(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal, 'incorrect', f'{cuid}_attn.npy'), attn_list)
                pbar.update(1)
...
if __name__ == "__main__":
    ...
    for goal in ['goal', 'belief', 'actions']:
        if os.path.exists(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal)):
            if input("Do you want to delete the previous vision_attn folder? (y/n): ").strip().lower() == 'y':
                shutil.rmtree(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal))
        os.makedirs(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal, 'correct'))
        os.makedirs(os.path.join(args.output_dir, 'attn', 'vision', model_name, goal, 'incorrect'))
    save_states(args)
